...
const navLinks = [
  ...
  { name: "Boutique", nameAr: "المتجر", href: "/boutique" },
  { name: "Jebba", nameAr: "جبة", href: "/boutique?category=jebba" },
  { name: "Keswa", nameAr: "كسوة", href: "/boutique?category=keswa" },
  { name: "Accessoires", nameAr: "إكسسوارات", href: "/boutique?category=accessoires" },
];
...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full">
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-20">
            ...
            <Link to="/" className="flex flex-col items-center">
              <h1 className="font-display text-2xl md:text-3xl font-bold text-primary tracking-wide">
                DAR EL BLED
              </h1>
              <span className="font-arabic text-sm text-tunisian-gold">دار البلاد</span>
            </Link>
      ...
      <div
        className={cn(
          "lg:hidden fixed inset-x-0 top-[calc(theme(spacing.20)+2.5rem+4px)] bg-background border-b border-tunisian-gold/20 transition-all duration-300 overflow-hidden",
        ...
        <nav className="container mx-auto px-4 py-4 flex flex-col gap-4">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              to={link.href}
              className="font-body text-lg font-medium text-foreground/80 hover:text-primary transition-colors py-2 border-b border-border/50"
